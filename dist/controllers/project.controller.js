...
class ProjectController {
    async create(req, res) {
        try {
            ...
            const { projectName, clientName, clientAddress, obraAddress, productId, startDate, status, } = req.body;
            // Validações básicas
            if (!projectName ||
                !clientName ||
                !clientAddress ||
                ...
                return res.status(400).json({
                    error: "projectName, clientName, clientAddress, obraAddress, productId and startDate are required",
                });
            }
            if (!userId) {
    ...
    async updateStepStatus(req, res) {
        try {
            if (!req.user) {
                return res.status(401).json({ error: "Unauthorized" });
            }
            ...
            const updatedProject = await projectService.updateStepStatus(projectId, scheduleId, status, actualDate);
            res.json(updatedProject);
        }
        catch (error) {
